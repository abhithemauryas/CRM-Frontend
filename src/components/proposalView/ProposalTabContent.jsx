...
const ProposalTabContent = () => {
  ...
  const fetchProposalProfile = async () => {
    try {
      ...
      // 🟢 If backend has description field, use it
      setValue(proposalData.description || "");
    } catch (error) {
      console.error("Error fetching proposal profile:", error);
      topTost(
        error?.response?.data?.message || "Error fetching proposal profile:",
        "error"
      );
    }
  };
                ...
                <div className="proposal-from">
                  ...
                  <div className="fs-13 text-muted lh-lg">
                    <div>
                      ...
                      <span>{proposal?.phone || "NA"}</span>
                    </div>
                    <div>
                      <span className="fw-semibold text-dark border-bottom border-bottom-dashed">
                        Email:
                      </span>
                      <span>{proposal?.email || "NA"}</span>
                    </div>
                    <address>
                      <span className="fw-semibold text-dark border-bottom border-bottom-dashed">
                        Address:
                      </span>
                      <span>{proposal?.addressLine1 || "NA"}</span>
                      <br />
                      {/* <span>{proposal?.addressLine2 || "—"},</span>
                      <br /> */}
                ...
                <div className="proposal-to">
                  ...
                  <div className="fs-13 text-muted lh-lg mt-3">
                    <div>
                      ...
                      <span>{proposal?.leadId?.phone || "—"}</span>
                    </div>
                    <div>
                      <span className="fw-semibold text-dark border-bottom border-bottom-dashed">
                        Email:
                      </span>
                      <span> {proposal?.leadId?.email || "—"}</span>
                    </div>
                    <address className="mb-0">
                      <span className="fw-semibold text-dark border-bottom border-bottom-dashed">
                        Address:
                      </span>
                      <span>{proposal?.addressLine1 || "—"}</span>
                      <br />
                      <span>
                        {proposal?.addressLine2 || "—"},{proposal?.city},
                        {proposal?.state}
                      </span>
                      <br />
                    </address>
          ...
          <div className="card stretch stretch-full">
            ...
            <div className="card-body p-0 proposal-tab-editor">
              <ReactQuill
                ...
                value={value}
                onChange={setValue}
                className=" font-inter botder-top-0"
              />
            </div>
          </div>
        </div>
